docker:=docker run --rm -v $(CURDIR):/app -w /app elegant-bro/money:7.4

build:
	docker build --build-arg VERSION=7.4 --tag elegant-bro/money:7.4 ./docker/

exec:
	docker run --rm -ti -v $(CURDIR):/app:rw -w /app elegant-bro/money:7.4 sh
...
coverage:
	$(docker) php check-coverage.php build/logs/clover.xml 100
